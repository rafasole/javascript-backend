...
const fs = require('fs');
const retornoErros = require('../erros/trataErros.js')
...
fs.readFile(textoPrompt,'utf-8',(erro, texto) => {
    ...
    try{
        if (erro) throw erro
        contagemDePalavras(texto)
    
    return
    
    }
    catch (erro){
        console.log(retornoErros(erro))
...
function contagemDePalavras(texto){
    const splitParagrafo = extraiParagrafos(texto);
    ...
    return console.log(paragrafoMap)
